...
import {transaksiKeluarColumns as columns} from "../data/data_keluar";
import {capitalize} from "/utils/utils";

const INITIAL_VISIBLE_COLUMNS = ["id_transaksi_keluar", "nama_barang", "tanggal", "harga_satuan", "jumlah", "total_harga", "actions"];

const TransaksiKeluarTable = ({data, onAddItemClick}) => {
    const [filterValue, setFilterValue] = useState("");
    const [selectedKeys, setSelectedKeys] = useState(new Set([]));
    const [visibleColumns, setVisibleColumns] = useState(new Set(INITIAL_VISIBLE_COLUMNS));
    const [rowsPerPage, setRowsPerPage] = useState(20);
    const [sortDescriptor, setSortDescriptor] = useState({column: "id_transaksi_keluar", direction: "ascending"});
    ...
    const filteredItems = useMemo(() => {
        ...
        if (hasSearchFilter) {
            filteredData = filteredData.filter((item) => item.barang.nama.toLowerCase().includes(filterValue.toLowerCase()));
        }
        return filteredData;
    }, [items, filterValue]);
    ...
    const handleDelete = useCallback(async (id_transaksi_keluar) => {
    try {
      const response = await fetch(`/api/transaksi_keluar?id=${id_transaksi_keluar}`, {
        method: 'DELETE',
      });

      if (response.ok) {
        setItems((prevItems) => prevItems.filter((item) => <item class="id_transaksi_keluar"></item> !== id_transaksi_keluar));
      } else {
    ...
    const handleBatchDelete = useCallback(async () => {
        ...
        try {
            const response = await fetch(`/api/transaksi_keluar`, {
                method: 'DELETE',
                headers: {
            ...
            if (response.ok) {
                setItems((prevItems) => prevItems.filter(item => !idsToDelete.includes(item.id_transaksi_keluar)));
                ...
                return (
                    <Button color="error" onClick={() => {
                        if (selectedKeys.size > 1) {
                            ...
                            handleDelete(item.id_transaksi_keluar);
                        }
                    }}>
                        Delete
                    </Button>
    ...
    return (
        <Table
            aria-label="Product out table with custom cells, pagination, and sorting"
            isHeaderSticky
        ...
        >
            ...
            <TableBody emptyContent={"No items found"} items={sortedItems}>
                {(item) => (
                    <TableRow key={item.id_transaksi_keluar}>
...
export default TransaksiKeluarTable;
